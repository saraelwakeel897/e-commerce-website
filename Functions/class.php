...
class Crud
{
    ...
    public function register($post)
    {
        ...
        if (isset($name) && !empty($name) && isset($email) && !empty($email) && isset($phone) && !empty($phone) &&  isset($password) && !empty($password) && isset($confirmPassword) && !empty($confirmPassword)) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    if (strlen($password) >= 8 && strlen($confirmPassword) >= 8) {
                        if ($password === $confirmPassword) {
                            ...
                            $query = "INSERT INTO `users` (`name`,`email`,`phone`, `password`,`active_status`,`license_date`) VALUES ('$name','$email','$phone','$password','$activationcode','$license_date')";
                            $sql = $this->con->query($query);
                            if ($sql == true) {
                                echo'<script>alert("Registration Successfully");
                                window.location.assign("../Registration/Registration.php");</script>';
                        ...
                        if (password_verify($password, $row['password'])) {
                            ...
                            if ($row['active_status'] !== "verified") {
                                ...
                                if ($sql == true) {
                                    ...
                                    window.location.assign("../login/Login.php");</script>';
        ...
        while($row = $res->fetch_assoc()){
            ...
            if($email == $row['email']){
               ...
               if($newpassword == $cnewpassword){
                  ...
                  $sql = $this->con->query($query);
                  if ($sql == true) {
                      header("Location:../login/Login.php?msg2=newPassword");
                  } else {
